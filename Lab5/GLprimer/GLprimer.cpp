...
int main(int, char*[]) {
   ...
   // glViewport(0, 0, width, height);  // The entire window
    ...
    Texture myTexture2;
    myTexture2.createTexture("textures/trex2.tga");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        std::array<GLfloat, 16> matR1 = mat4roty(M_PI/4 * glfwGetTime());
        std::array<GLfloat, 16> matvT = mat4translate(0.0f, 0.0f, -3.0f);
        std::array<GLfloat, 16> matBT = mat4translate(0.0f, 0.0f, -1.0f);
        std::array<GLfloat, 16> matP = mat4perspective(M_PI/4, 1.0f, 0.1f, 100.0f);
        //std::array<GLfloat, 16> matRx = mat4rotx(M_PI/4);

        //std::array<GLfloat, 16> matA = mat4mult(matRx, matR1);
        //std::array<GLfloat, 16> matE = mat4mult(matvT, matA);

        std:: array<GLfloat, 16> MV = mat4identity();
        std:: array<GLfloat, 16> MVinv = mat4identity();
        MV = mat4mult(MV, matvT);
        //MV = mat4mult(MV, matR1);
        //MV = mat4mult(MV, matRx);

        myKeyRotator.poll();
        myMouseRotator.poll();
        //std::array<GLfloat, 16> matRz = mat4rotz(myKeyRotator.phi());
        //std::array<GLfloat, 16> matRx = mat4rotx(myKeyRotator.theta());

        std::array<GLfloat, 16> matRzm = mat4rotz(myMouseRotator.phi());
        std::array<GLfloat, 16> matRxm = mat4rotx(myMouseRotator.theta());

        std::array<GLfloat, 16> matRzmInv = mat4rotz(-myMouseRotator.phi());
        std::array<GLfloat, 16> matRxmInv = mat4rotx(-myMouseRotator.theta());

        std::array<GLfloat, 16> matRxL = mat4rotx(myKeyRotator.theta());
        std::array<GLfloat, 16> matRyL = mat4roty(myKeyRotator.phi());

        //MV = mat4mult(MV, matRz);
        //MV = mat4mult(MV, matRx);


        MVinv = mat4mult(MVinv, matRzmInv);
        MVinv = mat4mult(MVinv, matRxmInv);

        GLint locationMV = glGetUniformLocation(myShader.id(), "E");
        ...
        GLint locationRxL = glGetUniformLocation(myShader.id(), "RxL");
        glUseProgram(myShader.id());  // Activate the shader to set its variables
        glUniformMatrix4fv(locationRxL, 1, GL_FALSE, matRxL.data());

        GLint locationRyL = glGetUniformLocation(myShader.id(), "RyL");
        glUseProgram(myShader.id());  // Activate the shader to set its variables
        glUniformMatrix4fv(locationRyL, 1, GL_FALSE, matRyL.data());

        GLint locationmatMVInv = glGetUniformLocation(myShader.id(), "MVinv");
        glUseProgram(myShader.id());  // Activate the shader to set its variables
        glUniformMatrix4fv(locationmatMVInv, 1, GL_FALSE, MVinv.data());
    ...
    MV = mat4mult(MV, matR1);
    MV = mat4mult(MV, matBT);
    MV = mat4mult(MV, matR1);

    MV = mat4mult(MV, matRzm);
    MV = mat4mult(MV, matRxm);
